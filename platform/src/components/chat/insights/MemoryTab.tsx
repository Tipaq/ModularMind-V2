...
import type { MemoryEntry, ContextData, BudgetOverview, KnowledgeData, KnowledgeChunk } from "@/hooks/useChat";
...
function MemoryDetailModal({ entry }: { entry: MemoryEntry }) {
...
// ── Memory Entry Card (inner, reused in card + modal trigger) ─

function MemoryCardInner({ entry }: { entry: MemoryEntry }) {
  const pct = Math.round(entry.importance * 100);
  const accent = TYPE_ACCENT[entry.memoryType] || { text: "text-muted-foreground", bg: "bg-muted-foreground", border: "border-border" };
...
export function MemoryTab({
  ...
  // Static allocations from model context window (no ref needed — only changes with model)
  ...
  // Current used values from budget overview or estimated from content
  ...
  // Sticky: remember last used token counts during streaming
  ...
  return (
    <div>
      {/* Context usage: history + memory + rag */}
