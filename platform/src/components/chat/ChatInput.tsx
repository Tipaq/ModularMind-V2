...
export function ChatInput({
  ...
  const selectedModel = chatModels.find(
    (m) => selectedModelId === m.id || selectedModelId === `${m.provider}:${m.model_id}`,
  );
                      ...
                      {Object.entries(
                        ...
                        <div key={provider}>
                          ...
                          {providerModels.map((m) => {
                            const engineId = `${m.provider}:${m.model_id}`;
                            return (
                              <DropdownMenuCheckboxItem
                                key={m.id}
                                checked={selectedModelId === engineId || selectedModelId === m.id}
                                onCheckedChange={() => onModelChange(engineId)}
                                className="text-xs"
                              >
