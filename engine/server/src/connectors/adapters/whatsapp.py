...
class WhatsAppAdapter(PlatformAdapter):
    ...
    async def verify_signature(
        ...
        if not app_secret:
            raise HTTPException(
                status_code=500,
                detail="WhatsApp app_secret not configured",
            )
        ...
        if not hmac.compare_digest(expected, signature_header):
            raise HTTPException(
                status_code=403, detail="Invalid WhatsApp signature"
            )
    ...
    async def send_response(
        ...
        if not access_token or not phone_number_id or not recipient:
            logger.warning(
                "WhatsApp missing access_token, "
                "phone_number_id, or recipient"
            )
            return
    ...
    def metadata(cls) -> ConnectorTypeMeta:
        return ConnectorTypeMeta(
            ...
            setup_steps=[
                ...
                "and enable WhatsApp product",
                "Under WhatsApp > API Setup, note the "
                "Phone Number ID and generate an access token",
                "Choose a verify token (any string) "
                "for webhook verification",
                "Fill in the credentials below and click Connect",
                "Copy the Webhook URL and paste it in the "
