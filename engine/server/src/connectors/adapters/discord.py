...
class DiscordAdapter(PlatformAdapter):
    ...
    async def handle_handshake(
        ...
        if payload.get("type") == 1:
            return HandshakeResult(is_handshake=True, response=JSONResponse(content={"type": 1}))
    ...
    def extract_message(self, payload: dict) -> ExtractedMessage | None:
        ...
        text = _extract_interaction_text(interaction_type, payload.get("data", {}))
        if not text:
            return None
    ...
    async def send_response(
        ...
        interaction_token = platform_context.get("interaction_token", "")
        if not application_id or not interaction_token:
            logger.warning("Discord followup missing application_id or interaction_token")
            return
        ...
        try:
            async with httpx.AsyncClient(timeout=10) as client:
                resp = await client.patch(
                    url, json={"content": response_text[:DISCORD_MESSAGE_LIMIT]}
                )
    ...
    def metadata(cls) -> ConnectorTypeMeta:
        return ConnectorTypeMeta(
            ...
            setup_steps=[
                ...
                'Under "OAuth2 > URL Generator", select bot + applications.commands scopes, '
                "then invite to your server",
                "Fill in the credentials below and click Connect",
                "Copy the Webhook URL below and paste it in "
                '"Interactions Endpoint URL" in your Discord app settings',
            ],
            fields=[
                ...
                ConnectorFieldDef(
                    key="channel_id", label="Channel ID", is_secret=False, is_required=False
                ),
            ],
        )
...
def _extract_interaction_text(interaction_type: int | None, data: dict) -> str | None:
