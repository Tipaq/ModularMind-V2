"""Post-execution hooks for scheduled tasks.

After a scheduled task execution completes, these hooks perform post-actions
...
async def _github_comment(
    ...
    """Comment on the PR with the task result."""
    ...
    body = (
        ...
        f"{summary}\n\n"
        f"---\n"
        f"*Automated by ModularMind: {config.name}*"
    )
...
async def _webhook(
    ...
    """POST to a webhook URL with the task result."""
    payload = {
        "scheduled_task_id": config.id,
        "scheduled_task_name": config.name,
        "run_id": run.id,
        "status": run.status.value,
