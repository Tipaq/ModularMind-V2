...
class MCPCatalogEntry(BaseModel):
    ...
    server_command: list[str] | None = Field(None, description="Command to run the MCP server inside the container")
    ...
    default_env: dict[str, str] | None = Field(None, description="Default environment variables injected at deploy time")
    required_secrets: list[CatalogSecret] = Field(default_factory=list)
    default_args: list[str] = Field(default_factory=list)
    documentation_url: str | None = None
    setup_flow: str | None = Field(None, description="Post-deploy setup flow type: 'qr-code' or None")
    ...
    def _check_runtime(self):
        ...
        if self.docker_image is not None and self.npm_package is None and self.server_command is None:
            raise ValueError("server_command is required when docker_image is set without npm_package")
        return self
...
class CatalogSecret(BaseModel):
    ...
    placeholder: str = ""
    required: bool = True
    is_secret: bool = Field(True, description="When False, dashboard renders as text input instead of password")
...
MCP_CATALOG: list[MCPCatalogEntry] = [
    ...
    MCPCatalogEntry(
        ...
        required_secrets=[
            CatalogSecret(key="NOTION_API_KEY", label="Notion Integration Token", placeholder="ntn_..."),
        ],
    ...
    MCPCatalogEntry(
        ...
        required_secrets=[
            CatalogSecret(key="POSTGRES_URL", label="PostgreSQL Connection URL", placeholder="postgresql://user:pass@host:5432/db"),
        ],
    ),
    MCPCatalogEntry(
        id="linear",
    ...
    MCPCatalogEntry(
        ...
        required_secrets=[
            CatalogSecret(key="JIRA_URL", label="Jira Instance URL", placeholder="https://your-org.atlassian.net"),
    ...
    MCPCatalogEntry(
        ...
        required_secrets=[
            CatalogSecret(key="GITHUB_TOKEN", label="GitHub Personal Access Token", placeholder="ghp_..."),
        ],
        documentation_url="https://github.com/modelcontextprotocol/servers",
    ),
    ...
    MCPCatalogEntry(
        ...
        required_secrets=[
            ...
            CatalogSecret(key="FIRECRAWL_API_URL", label="Firecrawl API URL (for self-hosted)", placeholder="https://api.firecrawl.dev", required=False, is_secret=False),
        ],
    ...
    MCPCatalogEntry(
        ...
        required_secrets=[
            CatalogSecret(key="TELEGRAM_BOT_TOKEN", label="Telegram Bot Token", placeholder="123456:ABC-..."),
        ],
    ...
    MCPCatalogEntry(
        ...
        required_secrets=[
            ...
            CatalogSecret(key="MS365_MCP_TENANT_ID", label="Azure AD Tenant ID", placeholder="common", required=False, is_secret=False),
        ],
    ...
    MCPCatalogEntry(
        ...
        required_secrets=[
            CatalogSecret(key="QDRANT_URL", label="Qdrant URL", placeholder="http://qdrant:6333", is_secret=False),
            CatalogSecret(key="COLLECTION_NAME", label="Collection Name (use mcp_ prefix to avoid conflicts)", placeholder="mcp_knowledge", required=False, is_secret=False),
        ],
    ...
    MCPCatalogEntry(
        ...
        server_command=["/app/.venv/bin/mcp-server-motherduck", "--db-path", ":memory:", "--read-write"],
        mem_limit="512m",
    ...
    MCPCatalogEntry(
        ...
        required_secrets=[
            ...
            CatalogSecret(key="GITLAB_URL", label="GitLab Instance URL (default: gitlab.com)", placeholder="https://gitlab.com", required=False, is_secret=False),
        ],
        documentation_url="https://github.com/modelcontextprotocol/servers",
    ),
    MCPCatalogEntry(
    ...
    MCPCatalogEntry(
        ...
        required_secrets=[
            ...
            CatalogSecret(key="CLOUDFLARE_ACCOUNT_ID", label="Cloudflare Account ID", is_secret=False),
        ],
    ...
    MCPCatalogEntry(
        ...
        required_secrets=[
            CatalogSecret(key="MONGODB_URI", label="MongoDB Connection URI", placeholder="mongodb://user:pass@host:27017/db"),
        ],
    ...
    MCPCatalogEntry(
        ...
        required_secrets=[
            CatalogSecret(key="ELASTICSEARCH_URL", label="Elasticsearch URL", placeholder="https://localhost:9200"),
    ...
    MCPCatalogEntry(
        ...
        required_secrets=[
            ...
            CatalogSecret(key="SHOPIFY_STORE_URL", label="Shopify Store URL", placeholder="https://your-store.myshopify.com", is_secret=False),
        ],
    ...
    MCPCatalogEntry(
        ...
        required_secrets=[
            CatalogSecret(key="CONFLUENCE_URL", label="Confluence Base URL", placeholder="https://your-org.atlassian.net/wiki", is_secret=False),
...
def get_free_catalog_entries() -> list[MCPCatalogEntry]:
    ...
    """
    return [
        entry for entry in MCP_CATALOG
        if all(not s.required for s in entry.required_secrets)
    ]
