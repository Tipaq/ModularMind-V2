...
class GraphCompiler:
    ...
    async def compile_graph(self, graph: GraphConfig) -> CompiledStateGraph:
        ...
        for node in graph.nodes:
            ...
            elif node.id in loop_target_ids and node.id not in {n.id for n in graph.nodes if n.type == "loop"}:
        ...
        if mcp_server_ids and self.mcp_registry:
            try:
                ...
                lc_tools, tool_executor = await discover_and_convert(
                    self.mcp_registry, mcp_server_ids,
                )
                if lc_tools:
                    logger.info(
    ...
    async def _create_tool_node(
        ...
        if tool_type == "mcp" and self.mcp_registry:
            ...
            async def mcp_tool_node(state: GraphState, runnable_config: RunnableConfig | None = None) -> dict:
            ...
            async def tool_node(state: GraphState, runnable_config: RunnableConfig | None = None) -> dict:
    ...
    def _create_parallel_node(
        ...
        async def parallel_node(state: GraphState, config: RunnableConfig | None = None) -> dict:
            ...
            results = await asyncio.gather(
                *[run_branch(bid) for bid in branch_ids]
            )
    ...
    def _create_merge_node(
        ...
        async def merge_node(state: GraphState, config: RunnableConfig | None = None) -> dict:
            ...
            logger.info("Merge %s: %d branches, strategy=%s", node_id, len(branch_results), strategy)
            ...
            if strategy == "concat_messages":
                ...
                for bid, output in merged_outputs.items():
    ...
    def _create_loop_node(
        ...
        async def loop_node(state: GraphState, runnable_config: RunnableConfig | None = None) -> dict:
            ...
            logger.info("Loop %s: %d items, mode=%s, target=%s", node_id, len(collection), mode, target_node_id)
            ...
            if mode == "batch":
                batch_state = {**state, "input_data": {**state.get("input_data", {}), "items": collection}}
        ...
        async def supervisor_node(
            ...
            if supervisor_agent_id:
                routing_result = await invoker.invoke(
                    supervisor_agent_id, state,
                    override_prompt=routing_prompt, config=config,
                )
            else:
            ...
            if json_match:
                try:
                    ...
                    selected = [
                        s for s in raw_selected
                        if s.get("id") in valid_worker_ids
                    ]
                    if len(selected) < len(raw_selected):
                        logger.warning(
                            "Supervisor %s: filtered %d invalid agent IDs",
                            node_id, len(raw_selected) - len(selected),
                        )
            ...
            if delegation_mode == "single" or len(selected) == 1:
                ...
                delegation_results = list(
                    await asyncio.gather(*[delegate(s) for s in selected])
                )
            ...
            if review_response and supervisor_agent_id:
                ...
                review_result = await invoker.invoke(
                    supervisor_agent_id, state,
                    override_prompt=review_prompt, config=config,
                )
