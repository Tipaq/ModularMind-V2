        ...
        <div className="border-t border-border/30 px-3 py-2 space-y-2">
          {tokens && (
            <div className="flex items-center gap-2 text-[11px]">
              <span className="font-mono text-muted-foreground">{formatK(tokens.prompt)}</span>
              <span className="text-muted-foreground/40">&rarr;</span>
              <span className="font-mono text-muted-foreground">{formatK(tokens.completion)}</span>
              <span className="text-muted-foreground/40">=</span>
              <span className="font-mono font-medium">{formatK(tokens.total)}</span>
              <span className="text-[10px] text-muted-foreground">tok</span>
              {tokens.estimated && (
                <span className="text-[9px] text-muted-foreground/60">(est)</span>
              )}
              {tokPerSec != null && (
                <span className="ml-auto text-[10px] text-muted-foreground">
                  <span className="font-mono font-medium text-foreground">{tokPerSec}</span> tok/s
                </span>
              )}
            </div>
          )}
          ...
          {llm?.messageCount && !llm.messageTypes && (
