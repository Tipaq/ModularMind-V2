...
export interface ChatInputProps {
  ...
  /** Custom label renderer for model items. Defaults to `display_name || name`. */
  modelLabel?: (m: EngineModel) => string;
  /**
   * Derive the ID passed to `onModelChange` when a model is selected.
   * Defaults to `(m) => m.id`. Platform passes `(m) => \`\${m.provider}:\${m.model_id}\``.
   */
                          ...
                          {providerModels.map((m) => {
                            ...
                            return (
                              <DropdownMenuCheckboxItem
                                ...
                                onCheckedChange={() => onModelChange(modelId)}
                                className="text-xs"
                              >
                                {modelLabel(m)}
                              </DropdownMenuCheckboxItem>
