...
export default function Graphs() {
  ...
  }, [newName, newDesc, createGraph, navigate]);
      ...
      render: (g) =>
        g.models && g.models.length > 0 ? (
          <div className="flex flex-wrap gap-1">
            {g.models.map((m) => (
              <span key={m} className="text-sm text-muted-foreground font-mono">
                {stripProvider(m)}
              </span>
            ))}
          </div>
    ...
    },
    {
      ...
      render: (g) =>
        g.models && g.models.length > 0 ? (
          <div className="flex flex-wrap gap-1">
            {g.models.map((m) => (
              <Badge key={m} variant="outline" className="text-[10px] py-0 px-1.5">
                {isLocalModel(m) ? "Local" : "Cloud"}
              </Badge>
            ))}
          </div>
        ) : (
