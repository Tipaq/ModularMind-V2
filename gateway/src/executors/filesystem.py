...
async def execute_filesystem(
    ...
    handler = {
        ...
        "read_multiple": _read_multiple,
        "list": _list,
        "list_with_sizes": _list_with_sizes,
        "tree": _tree,
        "info": _info,
        "search": _search,
        "write": _write,
    ...
    return await handler(args, sandbox_mgr, execution_id, agent_id, permissions)
...
async def _read(
    args: dict, sandbox_mgr: SandboxManager, execution_id: str,
    agent_id: str, permissions: Any,
) -> str:
    path = args.get("path", "")
    if not path:
    ...
    exit_code, output = await _exec(sandbox_mgr, execution_id, cmd, agent_id, permissions, is_safe=True)
    if exit_code != 0:
...
async def _read_media(
    ...
    _, mime = await _exec(sandbox_mgr, execution_id, mime_cmd, agent_id, permissions, is_safe=True)
...
async def _write(
    ...
    exit_code, output = await _exec(sandbox_mgr, execution_id, cmd, agent_id, permissions)
    if exit_code != 0:
...
async def _edit(
    ...
    exit_code, content = await _exec(
        sandbox_mgr, execution_id, f"cat {shlex.quote(path)}", agent_id, permissions,
    )
    if exit_code != 0:
    ...
    exit_code, output = await _exec(sandbox_mgr, execution_id, write_cmd, agent_id, permissions)
    if exit_code != 0:
